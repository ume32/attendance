...
<div class="correction-list__wrapper">
    ...
    <div class="tab-menu">
        <a href="{{ route('admin.corrections.index', ['status' => 'pending']) }}" class="{{ $status === 'pending' ? 'active' : '' }}">承認待ち</a>
        <a href="{{ route('admin.corrections.index', ['status' => 'approved']) }}" class="{{ $status === 'approved' ? 'active' : '' }}">承認済み</a>
    </div>
        ...
        <tbody>
            @foreach ($corrections as $correction)
                <tr>
                    ...
                    <td><a href="{{ route('admin.corrections.show', $correction->id) }}">詳細</a></td>
                </tr>
            @endforeach
        </tbody>
    </table>
